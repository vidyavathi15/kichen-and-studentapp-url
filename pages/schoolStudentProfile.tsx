...
const onClickSubmit= async () => {
    ...
    console.log("createFields", createStudentPayload)
    let  isValidPayload = createStudentFields.every((obj: {value: any , error:boolean}) => !["", null, undefined].includes(obj.value) && !obj.error) 

    if (isValidPayload){
        await axios.post("http://localhost:3000/api/users/studentApi", createStudentPayload).then((res: AxiosResponse) => {
            if (res.status === 200){
                console.log("res", res)
  ...
  return (
    <Box className={Styles.main}>
        <Typography variant="subtitle1" textAlign={"center"}>Student Profile</Typography>
...
<Grid item xs={12} md={12} lg={12} xl={12}>
    ...
    <Button variant="contained" sx={{width:"10%", textTransform:"none"}} size="medium" onClick={onClickSubmit}>Submit</Button>
    </Stack>
</Grid>
</Grid>
</Box>
